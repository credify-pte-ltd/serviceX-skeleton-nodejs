...
const checkAndCondition = (subconditions, user, usingScopes, checkValueFunc) => {
  let totalScopeNames = [];
  const validConditions = subconditions.filter((c) => {
    const { qualified, scopeNames } = check(c, user, usingScopes, checkValueFunc);
...
const checkNotCondition = (subconditions, user, usingScopes, checkValueFunc) => {
  if (subconditions.length !== 1) {
    return false;
  }
  const { qualified, scopeNames } = checkValueFunc(subconditions[0], user, usingScopes);
  return { qualified: !qualified, scopeNames };
};

const check = (condition, user, usingScopes, checkValueFunc) => {
  if (condition.kind === "AND_CONDITION") {
    return checkAndCondition(condition.subconditions, user, usingScopes, checkValueFunc);
  } else if (condition.kind === "OR_CONDITION") {
    return checkOrCondition(condition.subconditions, user, usingScopes, checkValueFunc);
  } else if (condition.kind === "NOT_CONDITION") {
    return checkNotCondition(condition.subconditions, user, usingScopes, checkValueFunc);
  } else {
    return checkValueFunc(condition, user, usingScopes);
  }
};

const evaluateOffer = (user, conditions, usingScopes, requiredCustomScopes, scopesDefinition, checkValueFunc) => {
  let level = 0;
  let usedScopes = [];
  let requestedScopes = [];
  try {
    requestedScopes = conditions.flatMap((c) => {
      if (c.subconditions) {
        return c.subconditions.flatMap((sb) => {
          if (sb.claim) {
            ...
            return lookUpScopeNameByClaimName(scopesDefinition, innerSb.claim.name);
          }
        })
      } else if (c.claim) {
    ...
    requestedScopes = [...requestedScopes, ...requiredCustomScopes].filter(onlyUnique);
  } catch (e) {
    console.log(e);
  }
  ...
  for (let i = conditions.length - 1; i >= 0; i--) {
    ...
    if (Object.keys(condition).length === 0) {
      ...
      const { qualified, scopeNames } = check(condition, user, usingScopes, checkValueFunc);
      if (qualified) {
        level = i + 1;
...
const personalizeOffers = (user, offers, scopeNames, scopesDefinition, checkValueFunc) => {
  const list = [];
  // offers are coming from API response, so it's using snake case.
  offers.forEach((offer) => {
    // This uses all the scopes
    const result = evaluateOffer(user, offer.conditions, scopeNames, offer.required_custom_scopes || [], scopesDefinition, checkValueFunc);
