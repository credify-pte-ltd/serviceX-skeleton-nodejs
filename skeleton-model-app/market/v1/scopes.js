...
const scopeNames = ["b09e8f99-6d89-4e7d-83ea-a43a1787b3e0:sendo-history-data", "b09e8f99-6d89-4e7d-83ea-a43a1787b3e0:sendo%20score"];
      ...
      "claims":[
        {
          ...
          "main_claim_id":"",
          "scope": { "name": "b09e8f99-6d89-4e7d-83ea-a43a1787b3e0:sendo-history-data" },
...
const composeClaimObject = (user, commitments) => {
  ...
  claims["b09e8f99-6d89-4e7d-83ea-a43a1787b3e0:sendo-history-data"] = {
    ...
    "b09e8f99-6d89-4e7d-83ea-a43a1787b3e0:sendo-history-data:commitment": commitments ? commitments["b09e8f99-6d89-4e7d-83ea-a43a1787b3e0:sendo-history-data"] : undefined
  };
  claims["b09e8f99-6d89-4e7d-83ea-a43a1787b3e0:sendo%20score"] = {
    ...
    "b09e8f99-6d89-4e7d-83ea-a43a1787b3e0:sendo%20score:commitment": commitments ? commitments["b09e8f99-6d89-4e7d-83ea-a43a1787b3e0:sendo%20score"] : undefined
  };
  return claims;
};
...
const checkValueCondition = (condition, user, usingScopes) => {
  const scopeName = common.lookUpScopeNameByClaimName(scopesDefinition, condition.claim.name);
  const userDeclined = !usingScopes.includes(scopeName);
  if (userDeclined) {
    return { qualified: false, scopeNames: [scopeName] };
  }

  if (condition.kind === "CONTAIN_CONDITION") {
    return { qualified: true, scopeNames: [scopeName] };
  }

  if (condition.kind === "IN_RANGE_CONDITION") {
    if (condition.claim.name === "3285592c-9aaf-4182-bff5-941ce5dac483:purchase-count") {
    ...
    if (condition.claim.name === "3285592c-9aaf-4182-bff5-941ce5dac483:total-payment-amount") {
      const qualified = user.totalPaymentAmount >= Number(condition.value) && user.totalPaymentAmount <= Number(condition.upper);
      return { qualified, scopeNames: [scopeName] };
    }
    if (condition.claim.name === "3285592c-9aaf-4182-bff5-941ce5dac483:sendo%20score") {
...
const personalizeOffers = (user, offers) => common.personalizeOffers(user, offers, scopeNames, scopesDefinition, checkValueCondition);
const evaluateOffer = (user, conditions, usingScopes, requiredCustomScopes) => common.evaluateOffer(user, conditions, usingScopes, requiredCustomScopes, scopesDefinition, checkValueCondition);
